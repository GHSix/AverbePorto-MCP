...
function isProtocol(input: string): boolean {
...
async function makeApiRequest(url: string, method: string, data: any, headers: any = {}, format: string = 'json') {
  try {
...
function extractSessionCookie(headers: any): string | null {
  ...
  async ({ sessionId, filePath, recipient, version }) => {
    ...
    if (response.body.success === 1) {
      ...
      if ((stats.R > 0 || stats.N > 0) && response.body.error) {
        ...
        errorMsg = `\nErrors: ${errors.map((e: { code?: number; msg?: string }) => e.msg || 'Unknown error').join(', ')}`;
      }
