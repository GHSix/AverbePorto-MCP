import { z } from 'zod';
import { makeApiRequest, handleApiError, isKey, isProtocol } from '../utils/logger.js';
...
interface ConsultProtocolParams {
  sessionId: string;
  keys?: string[];
  protocols?: string[];
  download?: boolean;
...
export const consultProtocolTool = {
  ...
  handler: async ({ sessionId, keys, protocols, download, delimiter }: ConsultProtocolParams) => {
    ...
    if (processedKeys.length === 0 && processedProtocols.length === 0) {
      throw new ValidationError(
        'No valid keys or protocols provided. Keys must be 44 characters and protocols 40 characters.',
        'keys/protocols',
        { keys, protocols }
      );
    }
