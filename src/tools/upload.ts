...
import { makeApiRequest, handleApiError } from '../utils/logger.js';
...
export const uploadTool = {
  ...
  handler: async ({ sessionId, filePath }: UploadParams) => {
    ...
    if (response.body && typeof response.body === 'object' && response.body.success === 1) {
      ...
      if ((stats.R > 0 || stats.N > 0) && response.body.error) {
        ...
        errorMsg = `\nErrors: ${errors.map((e: any) => e?.msg || JSON.stringify(e) || 'Unknown error').join(', ')}`;
      }
