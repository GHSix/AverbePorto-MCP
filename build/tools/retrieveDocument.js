...
export const retrieveDocumentTool = {
    ...
    handler: async ({ sessionId, modDoc, dtStart, dtLimit, dtType = 1, numDoc = '', emit = '', rem = '', exped = '', receb = '', dest = '', toma = '', importador = '', representante = '', page = 1, start = 0, limit = 25, prot = '', taxId = '', relation = 0, modal = 0, valid = 2 }) => {
        ...
        if (response.body && typeof response.body === 'object' && response.body.success === 1) {
            const docs = Array.isArray(response.body.S) ? response.body.S : [];
